 ...
 */
export const parsePosition = (positionString?: string): string[] => {
  ...
  tokens.forEach(token => {
    if (token === '↔️') { // Explicit absolute positioning
      ...
      if (value) classes.push(`left-[${value}px]`);
      ...
      const value = token.substring(2);
      if (value) classes.push(`top-[${value}px]`);
      ...
      const value = token.substring(2);
      if (value) classes.push(`rotate-[${value}deg]`);
    }
